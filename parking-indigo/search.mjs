...
function slotTimes(fromTime, toTime, step) {
  ...
  if (end < start) throw new Error('Horário final da varredura precisa ser igual ou posterior ao inicial.');
  const out=[];
...
async function main() {
  ...
  const exitTime=assertTime(env('INDIGO_EXIT_TIME','07:00'), 'Hora de saída');
  const fromTime=assertTime(env('INDIGO_FROM_TIME','06:00'), 'Horário inicial');
  ...
  const times=slotTimes(fromTime,toTime,step);
  if (times.length>97) throw new Error('Faixa de horários grande demais.');
  ...
  try {
    ...
    for(let i=0;i<times.length;i+=BATCH){
      ...
      const payloads=batch.map(time=>({
        time,
        body:{
          Criteria:[{
            LotId:LOCATION_ID,
            ParkingBeginDateTime:indigoDate(entryDate,time),
      ...
      const raw=await page.evaluate(async payloads=>Promise.all(payloads.map(async item=>{
        try{
          ...
          return {time:item.time,status:r.status,text:await r.text()};
        }catch(e){return {time:item.time,status:0,error:String(e),text:''};}
      })),payloads);

      for(const item of raw){
        if(item.status!==200){
          results.push({entry_date:entryDate,entry_time:item.time,exit_date:exitDate,exit_time:exitTime,status:'error',available:null,error:item.error||`HTTP ${item.status}`,booking_url:BOOKING_URL,products:[],alternatives:[]});
          continue;
        }
        try{
          ...
          results.push({
            entry_date:entryDate,entry_time:item.time,exit_date:exitDate,exit_time:exitTime,
          ...
          results.push({entry_date:entryDate,entry_time:item.time,exit_date:exitDate,exit_time:exitTime,status:'error',available:null,error:`Resposta inválida: ${e}`,booking_url:BOOKING_URL,products:[],alternatives:[]});
        }
      }
      if(i+BATCH<times.length) await page.waitForTimeout(350);
    }
  } finally {
    await browser.close();
  }

  results.sort((a,b)=>a.entry_time.localeCompare(b.entry_time));
  ...
  const payload={
    version:'1.0.0',status:'completed',generated_at:new Date().toISOString(),started_at:startedAt,request_id:requestId,
    location:{id:LOCATION_ID,name:'Aeroporto de Guarulhos (GRU)',booking_url:BOOKING_URL,timezone:'America/Sao_Paulo'},
    request:{entry_date:entryDate,exit_date:exitDate,exit_time:exitTime,from_time:fromTime,to_time:toTime,step_minutes:step,product},
    stats:{tested_times:results.length,available_times:available.length,unavailable_times:valid.length-available.length,error_times:results.length-valid.length,lowest_price:prices.length?Math.min(...prices):null},
    results
  };
  await fs.mkdir(path.dirname(OUTPUT_PATH),{recursive:true});
  await fs.writeFile(OUTPUT_PATH,JSON.stringify(payload,null,2),'utf8');
  console.log(`Indigo GRU: ${available.length}/${results.length} horários disponíveis para ${product}.`);
}

main().catch(async err=>{
  const payload={version:'1.0.0',status:'error',generated_at:new Date().toISOString(),request_id:env('INDIGO_REQUEST_ID','manual'),error:String(err?.stack||err)};
