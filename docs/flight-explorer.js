(() => {
  ...
  panel.innerHTML=`
    <div class="month-titlebar"><div><strong>🔎 Buscar passagens · v0.4.4</strong><small>Os achados são salvos durante a pesquisa. Se a execução parar, o que já foi encontrado continua disponível.</small></div><div class="pill"><span class="dot"></span><span id="monthSearchUpdated">Aguardando busca</span></div></div>
    <section class="panel">
  ...
  const monthInput=q('#monthValue');monthInput.min=monthKey(new Date(now.getFullYear(),now.getMonth(),1));monthInput.max=monthKey(new Date(now.getFullYear(),now.getMonth()+6,1));monthInput.value=monthKey(new Date(now.getFullYear(),now.getMonth()+1,1));
  ...
  q('#rangeStart').min=dateKey(new Date(now.getFullYear(),now.getMonth(),now.getDate()+1));q('#rangeStart').max=dateKey(new Date(now.getFullYear(),now.getMonth()+7,0));q('#rangeStart').value=dateKey(defaultRangeStart);
  ...
  function bridgeProgress(request){
    return new Promise(resolve=>{
      const callback='flightProgress_'+request.request_id,script=document.createElement('script');
  ...
  function liveProgress(live,elapsed){const s=live.stats||{},stage=live.stage||'google',total=Number(s.combinations||0),primary=Number(s.primary_completed??s.processed_combinations??0),primaryTotal=Number(s.primary_total||total),fb=Number(s.fallback_done||0),fbTotal=Number(s.fallback_total||0);let pct=4,label='Preparando pesquisa…',done=Math.min(primary,total),detail='✓ achados salvos';if(stage==='google'){pct=5+(primaryTotal?primary/primaryTotal:0)*67;label='Pesquisando combinações no Google Flights…';}else if(stage==='fallback'){pct=72+(fbTotal?fb/fbTotal:0)*24;label='Confirmando datas sem preço na segunda consulta…';detail=`✓ salvo · confirmação ${fb}/${fbTotal}`;}else if(stage==='completed'){pct=100;label='Pesquisa concluída';done=total;}else if(stage==='interrupted'){pct=total?Math.min(95,(primary/total)*70):0;label='Pesquisa interrompida — parcial preservado';}
    return{pct,stage:label,done,total,priced:Number(s.priced_combinations||0),remaining:Math.max(0,total-done),elapsed,detail};}

  async function fetchRunState(request,started,total){try{const runs=(await requestJson(ACTIONS_RUNS)).workflow_runs||[];const run=runs.find(x=>{const created=Date.parse(x.created_at||'');const title=String(x.display_title||'');return title.startsWith(`Busca ${request.request_id} ·`)&&created>=started-90000&&title.includes(`${request.dispatch_origin} → ${request.destination}`)&&title.includes(request.month)});if(!run)return null;activeRun=run;if(run.status==='queued')return{pct:3,stage:'Aguardando executor do GitHub…',done:0,total,priced:0,detail:'na fila',run};if(run.status==='completed'){if(run.conclusion==='success')return{pct:98,stage:'Finalizando resultado…',done:total,total,priced:Number(data.stats?.priced_combinations||0),detail:'publicando',run};if(run.conclusion==='cancelled')return{cancelled:true,run};return{failed:true,error:`A execução terminou com status ${run.conclusion||'erro'}.`,run};}const jobs=(await fetchJson(ACTIONS_RUN(run.id)))?.jobs||[],steps=jobs[0]?.steps||[],current=steps.find(s=>s.status==='in_progress')||[...steps].reverse().find(s=>s.status==='completed'),name=String(current?.name||'Preparando ambiente');return{pct:name==='Pesquisar matriz completa do período'?15:8,stage:name==='Dependências'?'Preparando motor de pesquisa…':'Preparando pesquisa…',done:0,total,priced:0,detail:name,run};}catch{return null}}

  async function pollSearch(request,started,total){const status=q('#monthSearchStatus');let lastActionsCheck=0,state=null;for(let i=0;i<675;i++){if(stopRequested)throw new Error('__STOPPED__');await sleep(4000);if(stopRequested)throw new Error('__STOPPED__');const elapsed=Math.round((Date.now()-started)/1000);const live=await fetchJson(LIVE_RAW),matchedLive=resultMatches(live,request,started);if(matchedLive){data=live;render();const p=liveProgress(live,elapsed);updateProgress(p);status.className=live.status==='partial'?'month-search-status bad':'month-search-status wait';status.textContent=live.status==='partial'?'⚠ A execução parou, mas todos os resultados encontrados até aqui foram preservados.':`🔎 ${p.stage} Os resultados já encontrados estão sendo salvos e exibidos abaixo.`;if(live.status==='completed'||live.status==='partial')return live;}
      if(Date.now()-lastActionsCheck>30000){lastActionsCheck=Date.now();const [a,bridge]=await Promise.all([fetchRunState(request,started,total),bridgeProgress(request)]);if(stopRequested)throw new Error('__STOPPED__');if(bridge?.status==='error')throw new Error(bridge.error||'O serviço recusou a pesquisa.');if(a)state=a;if(!state&&bridge&&bridge.status&&bridge.status!=='error'){const bs=String(bridge.status||'');state={pct:Number(bridge.percent||3),stage:bs==='queued'?'Pesquisa recebida pelo serviço…':bs==='completed'||bs==='done'?'Finalizando resultado…':'Pesquisa confirmada pelo serviço…',done:Number(bridge.completed||0),total:Number(bridge.total||total),priced:Number(bridge.priced||0),detail:bridge.message||'serviço confirmou o envio'};}if(a?.failed){if(data.results?.length){status.className='month-search-status bad';status.textContent='⚠ A execução terminou com erro, mas os achados parciais exibidos abaixo ficaram salvos.';return {...data,status:'partial'};}throw new Error(a.error)}if(a?.cancelled&&!stopRequested){if(data.results?.length)return {...data,status:'partial'};throw new Error('A pesquisa foi cancelada no servidor.')}}
      ...
      if(!matchedLive&&!state){
        updateProgress({pct:2,stage:'Aguardando confirmação do serviço…',done:0,total,priced:0,elapsed,detail:'envio ainda não confirmado'});
