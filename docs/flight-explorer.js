(() => {
  ...
  btn.className = 'tab'; btn.dataset.flightView = 'monthsearch'; btn.textContent = '📅 Buscar passagem';
  tabs.insertBefore(btn, tabs.children[1] || null);

  const panel = document.createElement('div');
  panel.id = 'flightMonthPanel'; panel.hidden = true;
  panel.innerHTML = `
    <div class="flight-head"><div><h3 class="section-title">📅 Busca por mês</h3><div class="sub">Escolha origem, destino e mês. O motor pesquisa a matriz completa: 01→02, 01→03, 01→04… 02→03, 02→04… até o último par possível do mês.</div></div><div class="flight-actions"><div class="pill"><span class="dot"></span><span id="monthSearchUpdated">Aguardando busca</span></div></div></div>
    ...
    </div><div class="month-search-status" id="monthSearchStatus">Escolha a rota e o mês. Em um mês de 31 dias são até 465 combinações de ida e volta.</div></section>
  ...
  function render(){const rows=data.results||[],stats=data.stats||{},req=data.request||{};q('#monthRows').innerHTML=rows.map((x,i)=>`<tr><td><span class="month-rank">${i+1}</span></td><td><b>${fmtDate(x.departure_date)}</b></td><td><b>${fmtDate(x.return_date)}</b></td><td>${Number(x.trip_days||0)} dias</td><td><b>${esc(x.airline||'Google Flights')}</b><small class="statline">${esc(x.stops||'')} ${x.duration?'· '+esc(x.duration):''}</small></td><td class="month-result-price">${money(x.price)}</td><td><span class="badge official">${esc(x.source_kind==='swoop'?'Swoop':'Google')}</span></td><td><a class="btn" href="${safe(x.url)}" target="_blank" rel="noopener">Ver voo</a></td></tr>`).join('');q('#monthEmpty').hidden=rows.length>0;q('#monthLowest').textContent=money(stats.lowest_price);q('#monthCombos').textContent=stats.combinations??'—';q('#monthPriced').textContent=stats.priced_combinations!=null?`${stats.priced_combinations} · ${stats.coverage_pct||0}%`:'—';q('#monthCount').textContent=rows.length?`${rows.length}/100`:'—';if(data.generated_at)q('#monthSearchUpdated').textContent=`Atualizado ${new Date(data.generated_at).toLocaleString('pt-BR')} · ${req.origin||''}→${req.destination||''}`;const daily=data.daily_min||[],cal=q('#monthCalendar');if(daily.length){const low=Math.min(...daily.map(x=>Number(x.price)||Infinity));cal.innerHTML=daily.map(x=>{const d=Number((x.departure_date||'').slice(-2)),hot=Number(x.price)<=low*1.06?' hot':'';return`<div class="month-day${hot}"><b>Dia ${d}</b><strong>${money(x.price)}</strong><small class="statline">volta ${fmtDate(x.return_date)} · ${Number(x.trip_days||0)} dias</small></div>`}).join('');q('#monthCalendarPanel').hidden=false}else{cal.innerHTML='';q('#monthCalendarPanel').hidden=true}}
  ...
  async function pollJob(jobId,started){const status=q('#monthSearchStatus');for(let i=0;i<600;i++){await sleep(2500);let r;try{r=await fetch(`${apiBase}/api/search/${encodeURIComponent(jobId)}?t=${Date.now()}`,{cache:'no-store',redirect:'follow'})}catch{throw new Error('Não foi possível consultar o servidor de busca.')}if(!r.ok)throw new Error(`Servidor respondeu ${r.status}.`);const j=await r.json(),elapsed=Math.round((Date.now()-started)/1000);if(j.status==='done')return j.result;if(j.status==='error')throw new Error(j.error||'A pesquisa falhou.');status.className='month-search-status wait';status.textContent=`🔎 Pesquisando todas as combinações do mês… ${elapsed}s. Você pode permanecer nesta página.`}throw new Error('A busca excedeu o tempo máximo de espera.')}
  async function searchInsidePage(){if(searching)return;const request=formRequest(),status=q('#monthSearchStatus'),button=q('#monthSearchButton');if(!/^[A-Z]{3}$/.test(request.destination)){status.className='month-search-status bad';status.textContent='Escolha uma cidade/aeroporto válido, por exemplo Miami ou MIA.';return}if(!request.month){status.className='month-search-status bad';status.textContent='Escolha o mês da viagem.';return}if(!apiBase){status.className='month-search-status bad';status.textContent='O Google Apps Script da pesquisa ainda não foi conectado. Publique o script como Web App e informe a URL /exec.';return}searching=true;button.disabled=true;button.textContent='⏳ Pesquisando…';status.className='month-search-status wait';status.textContent='🔎 Iniciando a matriz completa do mês…';const started=Date.now();try{const r=await fetch(`${apiBase}/api/search`,{method:'POST',redirect:'follow',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(request)});if(!r.ok){const t=await r.text();throw new Error(t||`Servidor respondeu ${r.status}.`)}const first=await r.json();data=await pollJob(first.job_id,started);render();status.className='month-search-status ok';status.textContent=`✅ Busca concluída em ${Math.round((Date.now()-started)/1000)}s. Foram consultadas ${data.stats?.combinations||0} combinações e as menores tarifas estão abaixo.`}catch(e){status.className='month-search-status bad';status.textContent=`Não foi possível concluir a busca: ${e.message||e}`}finally{searching=false;button.disabled=false;button.textContent='🔎 Pesquisar mês'}}
  ...
  q('#monthSearchButton').addEventListener('click',searchInsidePage);btn.addEventListener('click',activate);document.querySelectorAll('#flightTabs .tab').forEach(b=>{if(b!==btn)b.addEventListener('click',()=>{panel.hidden=true})});const productsMain=document.querySelector('.main-tab[data-main="products"]');if(productsMain)productsMain.addEventListener('click',()=>{panel.hidden=true});document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!searching)loadConfig()});boot();
})();