(() => {
  ...
  let data = {version:'0.2.3',generated_at:null,results:[],sources:{}};
  ...
  style.textContent = `
    ...
    .miles-status{padding:8px 10px;border-top:1px solid var(--line);color:var(--muted);font-size:11px;line-height:1.45}.miles-public-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:0 10px 10px}.miles-public-actions[hidden]{display:none}.miles-public-actions a{display:inline-flex;align-items:center;text-decoration:none;background:var(--accent);color:#07111f;font-weight:900;padding:8px 10px;border-radius:9px;font-size:11px}.miles-public-actions span{color:var(--muted);font-size:11px}.miles-status.ok{color:var(--ok)}.miles-status.warn{color:var(--warn)}.miles-status.bad{color:var(--hot)}
  ...
  panel.innerHTML = `
    <div class="miles-titlebar">
      <div><strong>⭐ Buscar passagens por milhas · v0.2.3</strong><small>Mesmo modelo de período da busca paga: mês inteiro ou intervalo de datas. A tabela exibe somente milhas.</small></div>
    ...
    <section class="panel">
      ...
      <div class="miles-status" id="milesStatus">Escolha a rota e o período para consultar os resultados em milhas.</div><div class="miles-public-actions" id="milesPublicActions" hidden></div>
    </section>
  ...
  function renderPublicFallback(){
    const box=q('#milesPublicActions');if(!box)return;
    const links=Array.isArray(data.fallback_urls)?data.fallback_urls:[];
    if(!data.fallback_mode||!links.length){box.hidden=true;box.innerHTML='';return;}
    box.hidden=false;
    box.innerHTML='<span>Busca pública disponível:</span>'+links.map(x=>`<a href="${safe(x.url||'')}" target="_blank" rel="noopener">↗ Seats.aero · ${esc(x.label||'abrir')}</a>`).join('');
  }
  ...
  async function loadData(){
    ...
    catch{data={version:'0.2.3',generated_at:null,results:[],sources:{}};}
  ...
  // A navegação principal precisa funcionar mesmo se um recurso secundário falhar.
  ...
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadData().then(()=>render(activeQuery)).catch(()=>{});});
  ...
  try{
    ...
    const paidOrigin=q('#monthOrigin');
    if(paidOrigin&&paidOrigin.value){
      const code=String(paidOrigin.value).trim().toUpperCase();
      const a=fallbackAirports().find(x=>x.code===code);
      if(a)chooseAirport('origin',a);else q('#milesOrigin').value=code;
    }
    loadWorldAirports();
    defaults();
    periodChanged();
    renderProviderActions(null);
    renderSources();
    loadConfig();
    loadData();
  }catch(err){
